...
const ALLOWED_LANGUAGES = [
	"en", "zh", "zh_TW", "ko", "ja", "fr", "es", "de", "it", "pt_BR", "ru", "tr","zh_Hans","zh_Hant","hi"
]
...
func _get_language_display(lang: String) -> String:
	if LANGUAGE_NAMES.has(lang):
		return "%s (%s)" % [LANGUAGE_NAMES[lang], lang.replace("pt_BR","pt").replace("zh_Hans","zh").replace("zh_Hant","zh")]
	return lang
